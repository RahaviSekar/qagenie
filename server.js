/**
 * QA Genie — Next.js UI + Express API on one port.
...
const flowsRoutes = require("./backend/routes/flows");
const regressionRoutes = require("./backend/routes/regression");
const { emitter } = require("./backend/services/logEmitter");
...
nextApp.prepare().then(() => {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const bodyBuf = Buffer.isBuffer(req.body) ? req.body : Buffer.from(JSON.stringify(req.body || {}));
        ...
        const ref = event.ref || "";
        if (String(req.get("x-github-event")) === "push" && (ref === "refs/heads/main" || ref === "refs/heads/master")) {
  ...
  http.createServer(app).listen(port, () => {
    console.log(`QA Genie → http://localhost:${port}  (${dev ? "dev" : "production"})`);
  });
});
